...
# Configure firwall rules for the Lighsail instance
...
resource "aws_iam_policy" "lightsail_snapshot_policy" {
    ...
    description = "Policy for creating and managing Lighsail snapshots"
...
resource "aws_iam_user_policy_attachment" "s3_backup_policy" {
    user = aws_iam_user.minecraft_backup.name
...
resource "aws_iam_access_key" "minecraft_backup" {
    user = aws_iam_user.minecraft.backup.name
}

# Generate provisioner script
data "template_file" "provisioner_script" {
    ...
    var = {
        ...
        backup_schedule_cron = var.snapshot_schedule_cron
        snapshot_schedule_cron = var.s3_backup_bucket_name
...
resource "null_resource" "provision_server" {
    ...
    triggers = {
        instance_id = aws_lightsail_instannce.minecraft_server.id
        disk_attachment = aws.lightsail_disk_attachment.minecraft_data.id 
        script_hash = sha256(data.template_file.provisioner_script.rendered)
    }
    ...
    provisioner "file" {
        content = data.template_file.provisioner_script.rendered
